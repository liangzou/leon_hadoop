...
//********************************test*************************      
    /*  byte [] m=key.getData();
      for(int i=key.getPosition();i<(keyLength+key.getPosition());i++){
    	  System.out.println("Class: IFile(no sep)  "+(char)m[i]);
      }
      byte [] v=value.getData();
      for(int i=value.getPosition();i<(valueLength+value.getPosition());i++){
    	  System.out.println("Class: IFile( no sep)  value:"+(char)v[i]);
      }*/
 //******************************test******************************     
   ...
   //*****************************************************************leon**************** 
    public void append(DataInputBuffer key, DataInputBuffer value, String separator)
    throws IOException {
      int keyLength = key.getLength() - key.getPosition();
      if (keyLength < 0) {
        throw new IOException("Negative key-length not allowed: " + keyLength + 
      ...
      //WritableUtils.writeVInt(out, keyLength);
      //WritableUtils.writeVInt(out, valueLength);
      out.write(key.getData(), key.getPosition()+1, keyLength-1);     
      out.write(separator.getBytes(utf8));
      out.write(value.getData(), value.getPosition()+1, valueLength-1); 
      //System.out.println("IFiles   :"+key.getData()+" value:"+value.getData());
      out.write(newline);
      // Update bytes written
      /*decompressedBytesWritten += keyLength + valueLength + 
                      WritableUtils.getVIntSize(keyLength) + 
   ...
   //******************************************leon******************************
	...
	/*public void append(Text key, Text value) {
