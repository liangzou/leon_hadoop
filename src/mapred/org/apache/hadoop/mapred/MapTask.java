...
class MapTask extends Task {
	...
	/**
	 * Yingyi: indicate whether this mapper is local or not
	 */
	private boolean localMapper = false;
		...
		private void writeSkippedRec(K key, V value) throws IOException {
			if (skipWriter == null) {
				...
				skipWriter = SequenceFile.createWriter(skipFile
						.getFileSystem(conf), conf, skipFile,
	...
	private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runOldMapper(
		...
		// re-instantiate the split
		try {
			inputSplit = (InputSplit) ReflectionUtils.newInstance(job
					.getClassByName(splitClass), job);
		...
		/*if(iteration>0){
			FileInputFormat ff=(FileInputFormat) job.getInputFormat();
			INKEY key=rawIn.createKey();
			INVALUE value=rawIn.createValue();
			Path[] path=ff.getInputPaths(job);
			while(in.next(key, value)){
				LOG.info("MapTask iteration:"+key +"value"+value+"job.getInputPath()"+path[0]+" splitClass"+splitClass);
			}
		}		*/
		...
		try {
			...
			for (int i = 0; i < locations.length; i++) {
				System.out.println("loc: " + locations[i] + " mapper host "
						+ hostname);
				if (locations[i].indexOf(hostname) >= 0
						|| fullname.indexOf((locations[i])) >= 0
						|| ipAddr.equals(locations[i])) {
					localMapper = true;
					break;
				}
			}

			conf.setCurrentIteration(this.iteration);
			conf.setCurrentStep(this.step);
			((MapRunner) runner).configure(conf, this.getTaskID(), localMapper);
	...
	private static class OldOutputCollector<K, V> implements
		...
		public void collect(K key, V value) throws IOException {
			try {
				collector.collect(key, value, partitioner.getPartition(key,
						value, numPartitions));
	...
	class MapOutputBuffer<K extends Object, V extends Object> implements
		...
		public synchronized void collect(K key, V value, int partition)
			...
			try {
				...
				//LOG.info("keySerializer.serialize(key); "+iteration+"  "+key);
						...
						if (kvstart == kvend) {
							...
							if (kvend != kvindex) {
								...
								if (bufsoftlimit || (buffull && !wrap)) {
									LOG
											.info("Spilling map output: buffer full= "
													+ bufsoftlimit);
									startSpill();
								}
		...
		private void sortAndSpill() throws IOException, ClassNotFoundException,
			...
			try {
				...
				LOG.warn("&&&&&Mapbufferoutputpath"+filename.toString()+"  &&&&");
		...
		private void mergeParts() throws IOException, InterruptedException,
			...
			for (int i = 0; i < numSpills; i++) {
				filename[i] = mapOutputFile.getSpillFile(mapId, i, round);
			...
			if (numSpills == 1) { // the spill is the final output
				...
				if (indexCacheList.size() == 0) {
					rfs
							.rename(mapOutputFile.getSpillIndexFile(mapId, 0,
			...
			Path finalIndexFile = mapOutputFile.getOutputIndexFileForWrite(
					mapId, finalIndexFileSize, round);
				...
				for (int parts = 0; parts < partitions; parts++) {
					...
					RawKeyValueIterator kvIter = Merger.merge(job, rfs,
							keyClass, valClass, codec, segmentList, job.getInt(
									"io.sort.factor", 100), new Path(mapId
